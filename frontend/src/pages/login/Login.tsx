      ...
      <main className='bg-secondary d-flex justify-content-around min-vh-100'>
        <section className='d-md-flex row w-100 justify-content-center'>
          <div className='col-6 d-flex p-0 align-self-lg-center justify-content-lg-center'>
            <form  className={`d-flex flex-column align-items-center justify-content-center gap-4 shadow rounded-5 ${Styles['shadow-sm-size']} ${Styles['bg-third-color']}`}>
              <img src={Logo} alt="Logo" className='w-50 pb-5 pt-4' />
              <input onChange={(e) => setLoginUser({...loginUser, name: e.target.value})} value={loginUser.name} type="text" placeholder='Enter your email' className={`rounded-5 w-75 p-2 placeholderColor form-control border-0 shadow-none ${Styles['placeholder']}`} />
              ...
              <button onClick={handleLogin} style={{textDecoration: 'none'}}  className={`w-75 rounded-5 btn-primary p-2 ${Styles['on-hover']} text-light border-0 font text-center `}>Login</button>
